...
export default function Chatbot(props) {
  ...
  useEffect(() => {
    ...
    const res2=await axios.post("http://localhost:3001/hi",{Email})

    
      // const reshis =  await axios.post("http://localhost:3001/history", Email);
      if (res2.data!==null) {
        
        setHistoryobj(res2.data) ;
        setHis(true)
        setVisiable('hide')
        // setVisiable(true)
      }else{
...
//     switchRefF1.current.classList.add('bg-black','text-white')
  ...
  return (
    <>
      ...
      {<div className=' bordr-4  border-red-800 flex w-full h-dvh overflow-hidden  bg-[rgb(249,249,249)]  '>
        ...
        <div className={`SIDE-NAV bordr relative z-50 bg-[rgb(249,249,249)]   transition-all delay-100 duration-100   bordr-black w-[18%] h-full flex flex-col items-center p-4 space-y-8 bg-opacity-80 max-[768px]:bg-opacity-100 bg-emerald00 shadw-2xl  max-[768px]:absolute
          ...
          <div className='SIDE_MENU w-6/6 m-auto  '>
            <ul className=' w-full p-2 text-lg font-medium font-semibld space-y-4 text-whit   max-lg:text-sm max-[768px]:text-md '>
              ...
              <Link to='/ai' className={`  text-${loc === "/ai" ? "black" : ""} hover:text-${loc === "/ai" ? "hover:text-red-500" : "hover:text-[rgb(114,116,118)]"} flex text-center items-center gap-2 `}>
              <BsRobot />
                    <p>AI</p>
                    </Link>
                    <p ref={rotate1}><IoMdArrowDropdown/></p>
                    </div>
                    <ul className='hidden flex flex-col gap-3  text-center  ' ref={subMenu}>
        ...
        <div className=' w-full relative  flex flex-row m-4 rounded-xl shadow-lg overflow-hidden'>
          <div className='AI-CHAT bordr bg-white  border-green-500 w-[80%] flex flex-col items-center h-full relative  m-auto overflow-auto max-[768px]:w-full  '>
              ...
              <div className='SWITCH_BTN   '>
                <div className=' flex justify-center gap-2 text-lg items-center  '>
                  <div className='border  flex rounded-md overflow-hidden max-sm:text-base  '>
                        <p className=' p-1 px-4 cursor-pointer  bg-black text-white  max-sm:px-2'   ><Link to={'/ai'}>CHAT</Link> </p>
                        <p className=' border h-auto '></p>
                        <p className='p-1 px-4 cursor-pointer max-sm:px-2' ref={switchRefT1} onClick={()=>{setSwitch1(()=>{return false})}} ><Link to={'/avaz'}>AVAZ</Link></p>
                  </div>
                   </div>
                </div>
              </p>
            ...
            <div className='INPUT w-full h-36 p-4 border-t border-neutral-300 max-md:h-20 items-center flex  '>
              <div className='INPUT-BAR max-h-24 bordr-2  bg-white w-[80%] flex flex-row justify-between  items-center    px-2 space-x-1 rounded-xl  m-auto p-2 shadow-lg border shadow-slate-200 max-sm:w-full   '>
                <button className=' text-xl   p-2 rounded-2xl hover:bg-gray-200 ' onClick={VoiceInputQuery}><FaMicrophoneAlt /></button>
                ...
                <textarea className='w-[85%] max-h-20  text-lg outline-none bg-white resize-none ' rows={1} onChange={inputQuery} value={input} placeholder='Ask Query' onKeyUp={increaseHieght}>
                </textarea>
                <button className=' text-xl  p-2 rounded-2xl hover:bg-gray-200 cursor-pointer ' disabled={input.length == 0} onClick={send}><IoSend /></button>
          ...
          <div  className={`HISTORY  overflow-yscroll overflow-xhidden  relative bg-white z-0 bordr bordr-blue-500 w-[27%] h-full p-6 border-l border-neutral-300 transition-all delay-100 duration-100 max-[768px]:w-[90%]   max-[768px]:absolute right-0 max-[768px]:${openHist ? ' translate-x-0' : 'translate-x-full '}`}>
          ...
          <button
                className=" md:hidden absolute top-[50%] -left-6 z-50  "
                onClick={history}
              >
                <IoMdArrowDropleft />
              </button>
           
           <div className=' top-0 w-full   p-2'><h1 className='text-3xl text-center justify-center  font-bold'>History</h1></div><hr className=' text-3xl bg-gray-400 text-gray-400  m-2 ml-0 w-full text-center '></hr>
