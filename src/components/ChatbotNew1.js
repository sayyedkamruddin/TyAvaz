...
eleA.firstElementChild.innerText=ans
  ...
  const send = async (e) => {
    ...
    <div className=' textwhite text-lg cursor-pointer opacity-50 hover:opacity-100 ' title='copy' onClick={()=>{copy(eleA)}}><MdContentCopy /></div>
    <div className=' textwhite text-lg cursor-pointer opacity-50 hover:opacity-100' title='reload' onClick={()=>{regenrate(eleU,eleA)}}><TbReload/></div>
    </>
