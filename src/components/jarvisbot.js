...
if (u.includes("open calendar")) {
  console.log(u)
    ...
    if (u.includes("write email for ")) {
      // readOut("opening twitter sir");
       ans = await fetchPost(u)
      const urlEnStr = await encodeURIComponent(ans);
      
      let a = window.open(`https://mail.google.com/mail/u/0/?fs=1&to=someone@example.com&su=SUBJECT&body=${urlEnStr}&bcc=someone.else@example.com&tf=cm`);
                ...
                <div className='SWITCH_BTN   '>
                  <div className=' flex justify-center gap-2 text-lg items-center   '>
                    <div className='border  flex rounded-md overflow-hidden max-sm:text-sm  '>
                      <p className=' p-1 px-4 cursor-pointer    max-sm:px-1'   ><Link to={'/ai'}>CHAT</Link> </p>
                      <p className=' border h-auto '></p>
                      <p className='p-1 px-4 cursor-pointer max-sm:px-1' ><Link to={'/avaz'}>AVAZ</Link></p>
                      <p className=' border h-auto '></p>
                      <p className='p-1 px-4 cursor-pointer bg-black text-white max-sm:px-1' ><Link to={'/jar'}>Jarvis</Link></p>
                    </div>
                  </div>
                </div>
              </p>
